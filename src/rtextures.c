...
Image GenImageWhiteNoise(int width, int height, float factor)
{
    ...
    {
        if (GetRandomValue(0, 99) < (int)(factor*100.0f)) pixels[i] = COLOR_WHITE;
        else pixels[i] = COLOR_BLACK;
    }
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing, Color tint)
{
    ...
    imText = GenImageColor((int)imSize.x, (int)imSize.y, COLOR_BLANK);
...
void ImageDither(Image *image, int rBpp, int gBpp, int bBpp, int aBpp)
{
    ...
    else
    {
        ...
        Color oldPixel = COLOR_WHITE;
        Color newPixel = COLOR_WHITE;
...
Color *LoadImagePalette(Image image, int maxPaletteSize, int *colorCount)
{
    ...
    if (pixels != NULL)
    {
        ...
        for (int i = 0; i < maxPaletteSize; i++) palette[i] = COLOR_BLANK;   // Set all colors to BLANK
...
void ImageDrawTextEx(Image *dst, Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
    ...
    ImageDraw(dst, imText, srcRec, dstRec, COLOR_WHITE);
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        else
        {
            ...
            faces = GenImageColor(size, size*6, COLOR_MAGENTA);
            ...
            for (int i = 0; i < 6; i++) ImageDraw(&faces, mipmapped, faceRecs[i], (Rectangle){ 0, (float)size*i, (float)size, (float)size }, COLOR_WHITE);
